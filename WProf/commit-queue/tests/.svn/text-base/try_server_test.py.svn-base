...
class TryServerTest(mocks.TestCase):
  def setUp(self):
    super(TryServerTest, self).setUp()
    ...
    try_server.TryRunner.update_latency = 0
    self.builders_and_tests = {
      'linux': ['test1', 'test2'],
      'mac': ['test1', 'test2'],
    }
    self.try_runner = try_server.TryRunner(
        self.context,
  ...
  def tearDown(self):
    ...
    super(TryServerTest, self).tearDown()
  ...
  def testNames(self):
    job = try_server.TryJob('builder', 123, ['test1'], False)
